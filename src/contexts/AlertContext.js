...
export const AlertProvider = ({ children }) => {
    ...
    const criticalAlerts = cityAlerts.filter((alert) => alert.severity === 'CRITICAL' && !alert.dispatched);
    // Recently dispatched alerts (for UI visibility)
    const recentDispatched = cityAlerts.filter((alert) => alert.dispatched && (new Date() - new Date(alert.dispatchedAt) < 600000) // 10 minutes
    );
