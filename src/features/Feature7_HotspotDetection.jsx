    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl shadow-lg">
      ...
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-2">
          ...
          📍 Feature 7: Hotspot Detection & Clustering
        </h2>
        <p className="text-gray-600 mt-2">
          159 hotspots identified via K-means clustering | High-crime density areas requiring targeted intervention
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="relative">
          <label className="block text-sm font-bold text-gray-700 mb-2">Search Hotspot</label>
          <div className="relative">
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6 flex justify-between items-center">
        ...
        <div>
          <p className="text-sm text-gray-600">Average Crime Density</p>
          <p className="text-2xl font-bold text-orange-600">
            {(filtered.reduce((sum, h) => sum + h.crimeDensity, 0) / filtered.length).toFixed(1)}%
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-600">Critical Hotspots</p>
          <p className="text-2xl font-bold text-red-600">{filtered.filter(h => h.priority === 'CRITICAL').length}</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {filtered.length > 0 ? (
            filtered.map((hotspot) => (
              <div
                key={hotspot.id}
                className={`p-4 rounded-lg border-l-4 bg-white shadow hover:shadow-lg transition ${
                  hotspot.priority === 'CRITICAL' ? 'border-l-red-600' :
                  hotspot.priority === 'HIGH' ? 'border-l-orange-600' :
                  hotspot.priority === 'MEDIUM' ? 'border-l-yellow-600' :
                  hotspot.priority === 'LOW' ? 'border-l-blue-600' : 'border-l-green-600'
                }`}
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-bold text-gray-800">{hotspot.name}</p>
                    <p className="text-sm text-gray-600">{hotspot.area}, {hotspot.city}</p>
                    <p className="text-xs text-gray-500 mt-1">📍 {hotspot.coords}</p>
                  </div>
                  <div className="text-right">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold text-white ${
                      hotspot.priority === 'CRITICAL' ? 'bg-red-600' :
                      hotspot.priority === 'HIGH' ? 'bg-orange-600' :
                      hotspot.priority === 'MEDIUM' ? 'bg-yellow-600' :
                      hotspot.priority === 'LOW' ? 'bg-blue-600' : 'bg-green-600'
                    }`}>
                      {hotspot.priority}
                    </span>
                    <p className="text-sm font-bold text-gray-800 mt-2">{hotspot.crimeDensity}%</p>
            ...
            <div className="text-center p-8 bg-white rounded-lg">
              <p className="text-gray-600">No hotspots found matching your search</p>
            </div>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.length > 0 ? (
            filtered.map((hotspot) => (
              <div
                key={hotspot.id}
                className={`p-4 rounded-lg border-t-4 bg-white shadow hover:shadow-lg transition ${
                  hotspot.priority === 'CRITICAL' ? 'border-t-red-600' :
                  hotspot.priority === 'HIGH' ? 'border-t-orange-600' :
                  hotspot.priority === 'MEDIUM' ? 'border-t-yellow-600' :
                  hotspot.priority === 'LOW' ? 'border-t-blue-600' : 'border-t-green-600'
                }`}
              >
                <p className="font-bold text-gray-800">{hotspot.name}</p>
                <p className="text-xs text-gray-600 mt-1">{hotspot.area}</p>
                <p className="text-xs text-gray-600">{hotspot.city}</p>
                <div className="mt-3 flex justify-between items-center">
                  ...
                  <p className="font-bold text-sm text-gray-800">{hotspot.crimeDensity}%</p>
                </div>
              </div>
            ))
          ) : (
            <div className="col-span-3 text-center p-8 bg-white rounded-lg">
              <p className="text-gray-600">No hotspots found matching your search</p>
            </div>
          )}
        </div>
      ...
      <div className="mt-6 p-4 bg-indigo-100 border-l-4 border-indigo-600 rounded">
        <p className="text-sm text-indigo-800">
          <strong>K-Means Clustering:</strong> 159 hotspots identified using K-means algorithm on crime location coordinates.
          Crime Density = density of crimes within a specific geographic radius. Showing {Math.min(filtered.length, 80)} of 159 total hotspots for demonstration.
          Each hotspot represents a high-crime concentration area requiring targeted police intervention.
        </p>
      </div>
    </div>
  );
