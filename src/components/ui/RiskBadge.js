...
export default function RiskBadge({ level, size = "default" }) {
    ...
    return (_jsxs("span", { className: cn("inline-flex items-center gap-1.5 rounded-full border font-medium", bg, text, border, size === "sm" ? "px-2 py-0.5 text-xs" : "px-3 py-1 text-sm"), children: [_jsx("span", { className: cn("rounded-full animate-pulse", level === "critical" ? "bg-red-400" : level === "high" ? "bg-orange-400" : level === "low" ? "bg-green-400" : "bg-yellow-400", size === "sm" ? "w-1.5 h-1.5" : "w-2 h-2") }), label] }));
}
