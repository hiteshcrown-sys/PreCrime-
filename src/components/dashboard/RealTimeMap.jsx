...
const RealTimeMap = ({ showHotspots = true, showCities = true, selectedCity = null, onCitySelect = null }) => {
  ...
  // Add city markers
  useEffect(() => {
    if (!mapReady || !map.current || !showCities) return;
  ...
  // Add hotspot markers
  useEffect(() => {
    if (!mapReady || !map.current || !showHotspots) return;
  ...
  return (
    <div className="w-full h-full bg-slate-900 rounded-lg overflow-hidden border border-slate-700">
      ...
      {/* Legend */}
      <div className="absolute bottom-4 right-4 bg-slate-800/90 border border-slate-600 rounded-lg p-3 max-w-xs">
        <h3 className="text-white font-semibold mb-3 text-sm">Risk Levels</h3>
        <div className="space-y-2">
          {['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'VERY_LOW'].map((level) => (
      ...
      {/* Info Box */}
      {selectedMarker && (
        <div className="absolute top-4 left-4 bg-slate-800/90 border border-cyan-500/30 rounded-lg p-4 max-w-xs">
          <div className="text-cyan-400 font-semibold">{selectedMarker}</div>
          <p className="text-slate-300 text-sm mt-1">Click on markers to view details</p>
        </div>
      )}
    </div>
  );
